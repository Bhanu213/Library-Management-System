...
public interface IssueRepository extends CrudRepository<Issue,Integer>{
	...
	@Query(value = "select * from user u,issue i where u.name = :name and i.status = :status",nativeQuery = true)
	public List<Issue> findIssueAllByUserNameAndStatus(@Param("name") String name,@Param("status") String status);
	
	@Query(value = "select * from book b,issue i where b.title= :title and i.status = :status",nativeQuery = true)
